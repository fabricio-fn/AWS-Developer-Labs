    ...
    try:
        ...
        if not pedido_id or not cliente_id:
            ...
            return {
                ...
                'body': json.dumps({'message': 'pedidoId e clienteId são obrigatorios'})
            }
        ...
        print("Erro: Corpo da requisição invalido, não e um JSON valido.")
        return {
            'statusCode': 400,
            'headers': {'Content-Type': 'application/json'},
            'body': json.dumps({'message': 'Corpo da requisição invalido, não e um JSON valido.'})
        }
