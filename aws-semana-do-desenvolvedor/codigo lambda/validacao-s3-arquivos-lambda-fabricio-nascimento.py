    ...
    for record in event['Records']:
        ...
        try:
            ...
            for s3_record in s3_notification['Records']:
                ...
                detalhes_erro_arquivo = "Conteudo do arquivo não e um JSON valido."
                arquivo_data = None
                ...
                try:
                    ...
                    # status_validacao_arquivo e detalhes_erro_arquivo ja estÃ£o definidos

                # Registrar no DynamoDB o status do ARQUIVO
                item_to_put = {
                    'nomeArquivo': object_key,
                    'timestamp': datetime.utcnow().isoformat() + "Z",
                    'statusValidacao': status_validacao_arquivo, # Status do arquivo, não dos pedidos individuais
                    'bucket': bucket_name
                }
            ...
            err_msg = f"Erro critico ao processar SQS record (body: {s3_event_message_body}): {str(e)}"
            if object_key: # Se ja tivermos o nome do arquivo
                err_msg = f"Erro cri­tico ao processar SQS record para arquivo {object_key} (body: {s3_event_message_body}): {str(e)}"
            print(err_msg)
            try:
                sns.publish(TopicArn=SNS_TOPIC_ARN, Message=err_msg, Subject="Erro Cri­tico no Processamento de Arquivo S3")
            except Exception as sns_e:
                print(f"Falha ao enviar notificação SNS sobre erro critico: {str(sns_e)}")
            raise e
