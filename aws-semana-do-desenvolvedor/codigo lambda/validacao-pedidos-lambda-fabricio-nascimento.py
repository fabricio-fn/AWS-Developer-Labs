    ...
    for record in event['Records']:
        try:
            ...
            if not pedido_data.get('itens') or not isinstance(pedido_data.get('itens'), list) or len(pedido_data.get('itens')) == 0:
                print(f"Erro de validação: Pedido {pedido_data.get('pedidoId')} não possui 'itens' validos.")
                # Para o lab, se falhar aqui, a mensagem sera removida da fila.
                # Se quiser que vÃ¡ para a DLQ da SQS, levante uma exceção: raise ValueError("Itens invalidos")
                continue
